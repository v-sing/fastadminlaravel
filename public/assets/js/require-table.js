define(['jquery', 'bootstrap', 'moment', 'moment/locale/zh-cn', 'bootstrap-table', 'bootstrap-table-lang', 'bootstrap-table-export', 'bootstrap-table-commonsearch', 'bootstrap-table-template'], function ($, undefined, Moment) {
    var Table = {
        ...
        api: {
             ...
             * table结构树
             */
            treeGrid: function (table, option) {
                option = $.extend(Table.treeGridOption, option || {})
                table.on('post-body.bs.table', function (e, settings, json, xhr) {
                    if (Fast.api.empty(settings)) {
                        return false;
                    }
                    var tr = table.find("tbody>tr");
                    var length = tr.length;
                    for (var i = 0; i < length; i++) {
                        tr.eq(i).addClass('treegrid-' + settings[i][option.treeId])
                        if (settings[i][option.treePid] == 0) {
                            tr.eq(i).addClass('treegrid-collapsed')
                        } else {
                            tr.eq(i).addClass('treegrid-parent-' + settings[i][option.treePid]);
                            tr.eq(i).hide()
                        }
                        if (settings[i]['tree'] == 1) {
                            tr.eq(i).addClass('treegrid-collapsed')
                        }
                    }
                })
                $(table).on("click", ".treegrid-expander", function (e) {
                    e.preventDefault();
                    var that = this;
                    tree(that);

                    function tree(that) {
                        var str = $(that).closest('tr').attr('class');
                        var Call = str.split(' ')[0].split('-')[1];
                        if (str.indexOf('treegrid-parent-' + Call) >= 0) {
                            return false;
                        }
                        var obj = $('.treegrid-' + Call + '>td>.treegrid-expander');
                        //折叠时
                        if (str.indexOf('treegrid-collapsed') != -1) {
                            $(that).closest('tr').removeClass('treegrid-collapsed');
                            $(that).closest('tr').addClass('treegrid-expanded');
                            $(that).removeClass('treegrid-expander-collapsed');
                            $(that).addClass('treegrid-expander-expanded');
                            $('.treegrid-parent-' + Call).show();
                            $('.treegrid-parent-' + Call).closest('tr').find('.treegrid-expander').each(function () {
                                show(this);
                            })
                        } else if (str.indexOf('treegrid-expanded') != -1) {
                            $(that).closest('tr').removeClass('treegrid-expanded');
                            $(that).closest('tr').addClass('treegrid-collapsed');
                            $(that).removeClass('treegrid-expander-expanded');
                            $(that).addClass('treegrid-expander-collapsed');
                            $('.treegrid-parent-' + Call).hide();
                            $('.treegrid-parent-' + Call).closest('tr').find('.treegrid-expander').each(function () {
                                hidden(this);
                            })
                        }
                    ...
                    function hidden(that) {
                       var class_m= $(that).attr('class');
                        ...
                        $(that).closest('tr').removeClass('treegrid-expanded');
                        $(that).closest('tr').addClass('treegrid-collapsed');
                        if(class_m.indexOf('treegrid-expander-expanded')!=-1&&class_m.indexOf('treegrid-expander-collapsed')!=-1){
                            $(that).removeClass('treegrid-expander-expanded');
                            $(that).addClass('treegrid-expander-collapsed');
                        }
