...
<head>
    ...
    <title>{{trans('admin::message.Reminder')}}</title>
...
<div class="system-message error">
    ...
    <h1>{{$msg}}</h1>
    <p class="jump">
        {{trans('admin::message.Page auto')}} <a id="href"
                                                 href="{{$url}}">{{trans('admin::message.Jump')}}</a> {{trans('admin::message.Waiting')}}
        ： <b id="wait">{{$wait}}</b>
    </p>
    <p class="clearfix">
        <a href="javascript:history.go(-1);" class="btn btn-grey">{{trans('admin::common.Go back')}}</a>
        <a href="{{$url}}" class="btn btn-primary">{{trans('admin::common.Jump now')}}</a>
    </p>
</div>
<div class="copyright">
